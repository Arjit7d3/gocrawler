...
> Source: the project repository. citeturn0view0
...
├── configure.go
├── crawl_page.go
├── extract_page_data.go
├── get_content.go
├── get_html.go
├── get_images_from_html.go
├── get_urls_from_html.go
├── csv_report.go
├── normalize_url.go
├── *_test.go   # unit tests for parsing/normalization
├── go.mod
└── go.sum
```

(See repository for the authoritative list.) citeturn0view0
